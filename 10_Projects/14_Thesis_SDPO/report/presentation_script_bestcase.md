...
Vấn đề đặt ra: một mô hình phải **tự giải một bài khó mà hiện tại nó chưa giải được**, và chỉ được cải thiện từ **chính các attempt của nó** cùng một **reward verifiable** (đúng/sai). Câu hỏi là làm sao để nó học được từ tình huống đó.

Có **hai trở ngại**. Thứ nhất — **tín hiệu thưa (sparse):** reward chỉ nói attempt đúng hay sai, **không** nói sai ở đâu, sai token nào; nên rất khó biết phải sửa chỗ nào — đây chính là bài toán *credit assignment*. Thứ hai — **không có gì để bắt chước:** trên một bài thật sự khó, mô hình gần như **không tự sinh ra nổi một attempt đúng nào**, nên chẳng có ví dụ đúng nào để học theo.

Hai trở ngại đó dẫn thẳng tới **flat-reward trap**: không có attempt đúng ⇒ không có gradient hữu ích ⇒ quá trình học **đứng yên đúng ở những bài khó nhất** — những bài ta quan tâm nhất. Mục tiêu của khóa luận là **tăng tốc quá trình khám phá (discovery)** này trên sinh mã phức tạp.

*(Lưu ý dẫn dắt: slide này cố ý nói theo cách chung, chưa nhắc method — flat-reward trap là vấn đề chung, đúng với mọi cách học từ chính mình. Slide 3 mới đưa setting cụ thể và SDPO vào.)*

---

## Slide 3 — Background: SDPO và khoảng trống (⏱ 1:00)

Trước hết là **bối cảnh cụ thể — test-time training (TTT):** cố định một bài khó, cho mô hình tự cải thiện qua vài bước ngay lúc inference, rồi reset; đo bằng **discovery@k** — nó tìm ra lời giải nhanh cỡ nào.

**SDPO** (Hübotter và cộng sự) xử lý **trở ngại thứ nhất** (tín hiệu thưa): thay vì chỉ dùng reward vô hướng, khai thác **rich feedback** mà môi trường verifiable vốn đã tạo ra — test fail, runtime error — làm tín hiệu **dày**. Mô hình khi được cho thêm feedback đóng vai **self-teacher**; phân bố per-token của nó được distill ngược lại vào student. Nhờ vậy **phân bổ tín hiệu học (credit assignment) dày hơn** GRPO: mỗi token có cả một phân bố top-K từ teacher, thay vì một con số cho cả chuỗi (nhiều hơn ~T·K lần thông tin, và có tín hiệu ngay cả khi chưa giải đúng).

**Khoảng trống** khóa luận nhắm tới: SDPO gốc là **student-first** — nó distill chính **rollout đã thất bại** của student. Mà trên bài khó thì gần như không có gì đúng để học → **vẫn dính flat-reward trap** (đúng trở ngại thứ hai ở slide 2). Đây chính là chỗ phương pháp đề xuất xử lý.
