...
Vấn đề đặt ra khá đơn giản: làm sao cải thiện một LLM ở tác vụ sinh code cho **một bài lập trình khó**, chỉ dựa vào **các attempt trước đó của chính nó** và **phản hồi từ môi trường** (chạy code, xem test nào fail / lỗi gì). Bài toán này **không mới** — nó thuộc dòng **RLVR** (Reinforcement Learning with Verifiable Rewards): phần thưởng do một verifier tự động cấp — chạy test cho code, khớp đáp án cho toán — thay vì reward model học từ human preference như RLHF. Trước đây nó được tiếp cận bằng các thuật toán policy-optimization quen thuộc, **tiêu biểu là GRPO** (cùng họ với PPO, DPO).

Nhưng cách tiếp cận đó vướng **hai trở ngại**. **Thứ nhất — tín hiệu thưa:** môi trường thật ra tạo ra phản hồi *giàu* (test nào fail, lỗi gì), **nhưng** các thuật toán này lại nén hết về một **reward đúng/sai duy nhất**; tín hiệu vì thế thưa — chỉ biết attempt đúng hay sai, không biết **sai ở token nào** (bài toán *credit assignment*). **Thứ hai — không có lời giải đúng nào để học:** đúng những bài khó thì mô hình gần như **không tự sinh nổi một lời giải đúng nào**, nên chẳng có gì đúng để mà **củng cố (reinforce, kiểu GRPO)** hay **bắt chước (imitate, kiểu distillation)**.

Ghép hai cái lại, ta có **flat-reward trap**: không có lời giải đúng ⇒ không có gradient hữu ích ⇒ việc học **đứng yên đúng ở những bài khó nhất** — chính những bài ta quan tâm. Mục tiêu của khóa luận là **tăng tốc quá trình khám phá (discovery)** lời giải này trên sinh mã phức tạp.

> *Nhắc: slide 2 chỉ nêu RLVR/GRPO như **bối cảnh**, hai trở ngại nói ở mức chung; còn setting cụ thể (test-time training) và cách xử lý (SDPO) để dành slide 3. Không claim đã benchmark trực tiếp DPO/PPO/GRPO.*
