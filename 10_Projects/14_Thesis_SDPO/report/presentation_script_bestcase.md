...
Vấn đề đặt ra khá đơn giản: làm sao cải thiện một LLM sinh code cho **một bài lập trình khó**, chỉ dựa vào **các attempt trước đó của nó** và **phản hồi từ môi trường**. Bài toán này không mới — nó thuộc dòng **RLVR** (Reinforcement Learning with Verifiable Rewards): phần thưởng do một verifier tự động cấp — chạy test cho code, khớp đáp án cho toán — thay vì reward model học từ human preference như RLHF. Trước đây nó được tiếp cận bằng các thuật toán policy-optimization quen thuộc, **tiêu biểu là GRPO** (cùng họ với PPO, DPO). Nhưng chúng đều vướng **hai trở ngại**: thứ nhất, tuy môi trường có phản hồi giàu, các phương pháp này lại nén về một **reward đúng/sai duy nhất** — tín hiệu thưa, không biết sai ở token nào (credit assignment). Thứ hai..." → _(dẫn sang trở ngại 2: bài khó thì gần như không sinh nổi lời giải đúng)_

Ở **hai chỗ**. Thứ nhất, tín hiệu **thưa**: reward chỉ nói attempt đúng hay sai, chứ không nói **sai ở đâu** — nên mô hình khó biết phải sửa token nào, đây chính là bài toán *credit assignment*. Thứ hai, **không có lời giải đúng nào để học**: đúng những bài khó thì mô hình gần như **không tự sinh nổi một lời giải đúng nào**, nên chẳng có gì đúng để mà **củng cố (reinforce, kiểu GRPO)** hay **bắt chước (imitate, kiểu SDPO)**.

Ghép hai cái lại, ta có **flat-reward trap**: không có lời giải đúng ⇒ không có gradient hữu ích ⇒ việc học **đứng yên đúng ở những bài khó nhất** — chính những bài ta quan tâm. Mục tiêu của khóa luận là **tăng tốc quá trình khám phá (discovery)** này trên sinh mã phức tạp.

> *Nhắc: slide này cố ý nói chung, chưa đụng SDPO/method — flat-reward trap đúng với mọi cách "học từ chính mình"; slide 3 mới đưa setting và SDPO vào.*

---

## Slide 3 — Background: SDPO và khoảng trống (⏱ 1:00)

Giờ vào **bối cảnh cụ thể**: đây là **test-time training** — cố định một bài khó, cho mô hình tự cải thiện qua vài bước ngay lúc inference rồi reset; ta đo bằng **discovery@k**, tức nó tìm ra lời giải nhanh cỡ nào.

**SDPO** (Hübotter và cộng sự) chính là lời giải cho **trở ngại thứ nhất** — tín hiệu thưa. Thay vì chỉ dùng reward vô hướng, nó khai thác **rich feedback** mà môi trường verifiable vốn đã tạo ra (test fail, runtime error) làm tín hiệu **dày**: mô hình khi được cho thêm feedback đóng vai **self-teacher**, và phân bố per-token của self-teacher được distill ngược vào student. Nhờ vậy **credit assignment dày hơn** GRPO — mỗi token một phân bố top-K từ teacher, thay vì một con số cho cả câu (nhiều hơn ~T·K lần thông tin, và có tín hiệu ngay cả khi chưa giải đúng).

Nhưng còn **khoảng trống**: SDPO gốc là **student-first** — nó distill chính **rollout đã thất bại** của student. Mà trên bài khó thì gần như không có gì đúng để học, nên **vẫn dính flat-reward trap** — đúng trở ngại thứ hai ở slide trước. Đây chính là chỗ phương pháp đề xuất nhắm vào.
