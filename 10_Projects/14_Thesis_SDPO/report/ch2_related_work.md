...
created: 2026-06-26
updated: 2026-06-26
tags: [report, chapter, related-work, background, sdpo, rlvr, kim, sdft]
sources: [src_hubotter2026_self_distillation, src_kim2026_why_self_distillation_degrades, src_shenfeld2026_sdft, con_sdpo_loss_mechanics, con_discovery_at_k, syn_template_taxonomy_rationale]
---
...
```mermaid
flowchart LR
    subgraph G[GRPO: sparse]
      GR[scalar reward r] --> GA[one advantage per sequence]
      GA --> GT[same signal copied to every token]
    end
    subgraph S[SDPO: dense]
      SFB[feedback f] --> ST[teacher per-token distribution]
      ST --> STK[soft target over top-K vocab at each position]
    end
```

**Figure 2.1**
*Credit-assignment density, GRPO versus SDPO.* GRPO attaches one scalar advantage to a whole sequence, so every token receives the same signal ($O(1)$). SDPO conditions the teacher on feedback and supplies a soft, $K$-dimensional target at each position ($O(T\cdot K)$). Schematic; see the table above for the formal density ordering.
...
Numbered per the master bibliography (`00_references.md`): [1] Hübotter et al. 2026 · [2] Kim et al. 2026 · [3] Shenfeld et al. 2026 · [4] lasgroup SDPO repo · [6] LiveCodeBench · [8] GRPO / DeepSeekMath · [9] Chen et al. 2021 · [11] Agarwal et al. 2023 · [12] Dolan & Moré 2002 · [13] Hinton et al. 2015.
