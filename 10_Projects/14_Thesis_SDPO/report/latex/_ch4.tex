...
\textbf{TTT protocol.} Per problem, the model is reset to base, then run for \(K\) steps; the student is evaluated PRE and POST with no context (§3.6). Code: \(K=15\), eval 16 samples, \passthrough{\lstinline!teacher\_n=10!}. Math: \(K=3\), eval 4 samples, \passthrough{\lstinline!teacher\_n=4!}, \passthrough{\lstinline!max\_new\_tokens=16384!}. The distillation core is identical to §3.3.5 (AdamW lr=1e-5, top-K=20, \(\alpha=1.0\) reverse KL, IS clip 2.0). Full hyperparameters are in Appendix A.
...
\begin{center}\includegraphics[width=0.85\linewidth]{../figures/fig_4_1_main_result.pdf}\end{center}
...
\begin{center}\includegraphics[width=0.85\linewidth]{../figures/fig_4_2_per_seed_slope.pdf}\end{center}
...
\begin{center}\includegraphics[width=0.85\linewidth]{../figures/fig_4_3_escape_zero.pdf}\end{center}
...
Beyond aggregate pass-rate, the runs show genuine discovery rather than confidence inflation. On idx12, greedy decoding moves 0.075 → 1.0; on idx39, 0.175 → 0.225. The model learns to solve problems it deterministically failed before. One concrete trace (full excerpt in Appendix D.5): idx12 base uses \passthrough{\lstinline!math.factorial(X)!} in the wrong direction, the feedback reports a runtime error, and the model learns to iterate to find N. In an exploratory run (idx29), the base calls \passthrough{\lstinline!exit()!} (unavailable in the sandbox), the feedback reports a \passthrough{\lstinline!NameError!}, and the model corrects it.

Two further observations. On easy-to-fix exploratory problems (idx23, idx29 at eval-8), both arms solve almost equally, so differentiation is clearer on harder problems. And from around step 2 the teacher tends to converge (batch reward = 1.0, mean similarity ≈ 0.9, \passthrough{\lstinline!n\_good!} → 1); when the base starts high, both arms can over-distill, consistent with the idx39 seed-3 control above.

\subsection{4.2.5 Compute-matched comparison (RQ3)}\label{compute-matched-comparison-rq3}

\begin{quote}
⚠ \textbf{Placeholder data.} The compute-matched student-first (\(g{=}10\)) numbers in this subsection are \emph{synthetic placeholders}, produced for format and framing review while the Modal/Colab compute quota is unavailable. The real runs are scheduled after the quota reset (August 2026); every value marked \textbf{†} must be replaced before submission. See \passthrough{\lstinline!PLACEHOLDERS.md!}.
\end{quote}

The comparisons in §4.2.2--§4.2.3 hold the step budget fixed but not the number of generations: teacher-first draws ten samples per step (\passthrough{\lstinline!teacher\_n=10!}) against student-first's four, roughly a 2.5× overhead. To test whether the advantage is merely a sampling-volume effect, we re-run student-first at a matched budget of ten generations per step (\passthrough{\lstinline!num\_generations=10!}) and compare against teacher-first at the same budget. Mean POST pass@16:
...
\bottomrule\noalign{}
\endlastfoot
idx39 & 0.094 & 0.13 \textbf{†} & 0.172 \\
idx12 & 0.844 & 0.92 \textbf{†} & 1.000 \\
idx64 & 0.047 & 0.11 \textbf{†} & 0.422 \\
idx77 & 0.203 & 0.27 \textbf{†} & 0.344 \\
\end{longtable}
...
\begin{center}\includegraphics[width=0.85\linewidth]{../figures/fig_4_4_template.pdf}\end{center}
...
The independence judge can be a cheap string-similarity check (difflib) or a semantic LLM judge (groq \passthrough{\lstinline!llama-3.3-70b!}). Mean POST pass over 4 seeds, good\_only:
...
TF ≥ SF under both judges on both problems, and the two judges give similar student POST, so the core result is \textbf{invariant to the judge choice}. The two judges do label differently: the semantic LLM calls fewer trajectories ``copy'' than difflib (on idx12 the LLM keeps \passthrough{\lstinline!n\_good!} up to 10 versus 1 for difflib), but the \emph{outcome} is unchanged, because the update is driven by correctness and convergence rather than by the exact copy boundary. As noted in §3.3.3, with thinking OFF the judge's \passthrough{\lstinline!reasoning\_quality!} saturates at 4--5, so it functions only as a copy-detector; no claim is made about reasoning quality.
...
\begin{center}\includegraphics[width=0.85\linewidth]{../figures/fig_4_5_ablation.pdf}\end{center}
...
Gemma-4-E4B (thinking ON) on MathArena AIME 2026 {[}10{]} (30 problems, integer answers), \passthrough{\lstinline!reference\_mode = ground\_truth!} (the teacher always sees the correct answer, the extreme leak regime), LLM judge \passthrough{\lstinline!glm-4.5-flash!} with a derive-vs-copy prompt. A frontier scan (n\_samples = 2) spreads the problems out: frontier (0 \textless{} pass \textless{} 1) at idx 8, 11, 21, 25; too-hard (pass = 0) at 15 problems; ceiling (pass = 1) at 11 problems. Gemma-4 with thinking solves many AIME 2026 problems, so difficulty is scattered rather than monotone.

\begin{center}\includegraphics[width=0.85\linewidth]{../figures/fig_4_7_frontier_bands.pdf}\end{center}
...
\begin{center}\includegraphics[width=0.85\linewidth]{../figures/fig_4_6_math_discovery.pdf}\end{center}
...
Numbered per the master bibliography (\passthrough{\lstinline!00\_references.md!}): {[}1{]} Hübotter et al.~2026 · {[}2{]} Kim et al.~2026 · {[}7{]} Qwen3 · {[}10{]} MathArena AIME 2026. (Full method and metric definitions in Chapter 3.)
