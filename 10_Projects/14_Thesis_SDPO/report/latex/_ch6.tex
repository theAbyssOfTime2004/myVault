...
Teacher-first draws roughly 2.5× more generations per step than student-first (\passthrough{\lstinline!teacher\_n=10!} versus four). The thesis therefore demonstrates an outcome advantage, not a compute advantage. A fair comparison requires a compute-to-correct view that holds total generation budget fixed, which was not run. The escape-zero events argue that volume alone does not explain the gap (§5.1), but they do not settle the compute question.
