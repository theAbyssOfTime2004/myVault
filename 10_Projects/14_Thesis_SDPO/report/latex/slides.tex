...
\institute[VNUHCM-US]{VNUHCM University of Science \\ \small Supervisor: Ng\^{o} Minh M\~{\^{a}}n}
\date{2026}
...
\begin{itemize}
  ...
  \item[\textbf{RQ3}] What are the \emph{structural \& domain limitations} going from procedural code to value-only math?
\end{itemize}
\end{frame}
...
% 10
\begin{frame}{Compute-fair: not just more sampling (RQ-method)}
\begin{columns}
...
% 12
\begin{frame}{RQ3: value vs.\ procedure --- when it works}
\begin{columns}
\column{0.58\textwidth}
...
\begin{enumerate}
  ...
  \item Teacher-first \textbf{significantly accelerates discovery} and \textbf{escapes the flat-reward trap} --- compute-fair (RQ-method).
...
\begin{itemize}
  \item One \textbf{4B} Qwen family at a personal compute scale --- a characterization, not a scaling law.
  \item Math pilot bottlenecked by \textbf{AIME}'s answer-only feedback (no worked solutions to derive from).
  \item A stronger base (\textbf{8B+}) is expected to \emph{narrow} the TF-vs-SF margin on reachable tasks.
  \item Epistemic result tied to one model family / lexical marker set.
\end{itemize}
\vfill
\textbf{Roadmap (post-graduation):} method-bearing math references (\textbf{MATH-500}) to lift the math bottleneck; scale to \textbf{8B+} and a second code benchmark; broaden the epistemic measurement across tokenizers and scales.
\end{frame}
