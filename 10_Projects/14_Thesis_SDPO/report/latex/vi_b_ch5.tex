...
Nghiên cứu này bổ sung thêm một điều kiện phụ thuộc regime cho phê phán về epistemic suppression, được trình bày như một cách diễn giải chứ chưa phải một cơ chế đã chứng minh. Trong miền code, nơi privileged reference là một chương trình hợp lệ về mặt cấu trúc, sự đè nén ở mức token \emph{đồng xảy ra} (co-occur) với việc functional correctness được cải thiện. Vì correctness được đo trên held-out test, mức tăng này phản ánh một phương pháp tổng quát hóa được, chứ không phải một đáp án được chép sẵn; cách đọc tự nhiên nhất là cả sự đè nén lẫn mức tăng correctness đều là hệ quả của việc củng cố một thủ tục đúng — chứ không phải sự đè nén là nguyên nhân gây ra mức tăng đó. Như Kim và cộng sự {[}2{]} từng chỉ ra, trong math leak regime, cùng một sự đè nén bề mặt lại nhất quán với việc sao chép đáp án, đơn giản vì reference chỉ là một giá trị trơ, không có thủ tục nào để mà nội hóa cả. Theo cách đọc này, sự đè nén không mặc nhiên có hại; ý nghĩa của nó phụ thuộc vào việc privileged context có mang một phương pháp tái lập được hay chỉ là một giá trị thô. Để cô lập loại reference như nguyên nhân thực sự tác động, cần một ablation cùng-miền (một reference chỉ-có-giá-trị so với một reference mang-phương-pháp, cùng trên code) — điều này được để lại cho hướng phát triển sau.

\section{Vị trí so với bài báo gốc}\label{position-relative-to-the-parent-paper}

Công trình này định vị mình đối chiếu với formulation của Hübotter và cộng sự {[}1{]}. Chúng tôi đề xuất một tổ chức teacher-first của execution-guided self-distillation, tăng tốc test-time discovery dưới một giao thức đánh giá compute-fair. Cùng với việc đo hành vi epistemic trong tối ưu code và đặc trưng hóa ranh giới value-versus-procedure, nó góp phần giải quyết một số câu hỏi vận hành mà bài báo gốc từng để ngỏ.
